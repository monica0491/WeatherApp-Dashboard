...
function success(position) {
    ...
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lon + "&APPID=06446a5e36dce564e1a5ba68c5164af0";
    $.ajax({
        url: queryURL,
        method: "GET"
...
function showPrevious() {
    ...
    if (savedLocations) {
        ...
        for (var i = 0; i < savedLocations.length; i++) {
            var locBtn = $("<p>").text(savedLocations[i]);
...
function getCurrent(city) {
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=06446a5e36dce564e1a5ba68c5164af0&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET",
        error: function (){
            savedLocations.splice(savedLocations.indexOf(city), 1);
            localStorage.setItem("city-search", JSON.stringify(savedLocations));
            initialize();
        }
    }).then(function (response) {
        //create the card
        var currCard = $("<div>").attr("class", "card bg-light");
        $("#current-weather").append(currCard);
        ...
        //get UV Index
        var uvURL = "https://api.openweathermap.org/data/2.5/uvi?appid=06446a5e36dce564e1a5ba68c5164af0&lat=" + response.coord.lat + "&lon=" + response.coord.lat;
        $.ajax({
            url: uvURL,
            method: "GET"
...
function getForecast(city) {
    //get 5 day forecast
    var queryURL = "https://api.openweathermap.org/data/2.5/forecast?id=" + city + "&APPID=06446a5e36dce564e1a5ba68c5164af0&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET"
    }).then(function (response) {
